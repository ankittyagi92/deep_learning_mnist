...
https://github.com/MehdiSv/TrafficSignsRecognition/blob/master/.ipynb_checkpoints/final_Traffic_Signs_Recognition-checkpoint.ipynb
...
#ideally a stratified split, or a randomised split should be used
...
#this was the most stable model when compared on the validation set
#across 5 different models
...
score = model.evaluate(test_img, test_cl, verbose=1)
pred_cl = model.predict_classes(test_img)
...
#make predictions for submission on test data
validation= pickle.load(open("testing_set.pkl", "rb"))
#plt.imshow(validation[28]["img"])
#validation[28]["img"].shape
#len(validation)

#transform validation set as well
val_img=[]
for ele in validation:
    val_img.append(ele["img"])

#val_img[28].shape

for i in range(0,len(val_img)):
    val_img[i]= imresize(val_img[i],(im_w,im_h))
    val_img[i] =val_img[i]/255.

val_img_np= np.array(val_img)

if K.image_dim_ordering() == 'th':
    val_img_np = val_img_np.reshape(val_img_np.shape[0], 3, im_h, im_w)
else:
    val_img_np = val_img_np.reshape(val_img_np.shape[0], im_h, im_w, 3)
    
pred_val = model.predict_classes(val_img_np)
